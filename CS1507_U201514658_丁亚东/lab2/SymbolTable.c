...
GlobalScope AddIntoGlobal(GlobalScope global_symtable, char* name, DecafCategory category, char* type, int num, ...)
{
    ...
    if(num == 1){
    	...
    	ClassDepictor d = va_arg(variables, ClassDepictor);
    	tmp->depictor.classdsp = d;
    }
    else{
    	tmp->depictor.classdsp = NULL;
    }
...
void AddThisType(ClassScope class_symtable, char* class_name)
{
    ...
    {
        if (class_symtable->category == Function && class_symtable->depictor.funcdsp->isstatic == 0)
        {
...
{
	FormalScope tmp = depictor.funcdsp->formalscope;
	if(tmp->next)
	{
		depictor.funcdsp->formalscope = tmp->next;
		free(tmp);
	}
	else{ //把this改为nil
		char* cache = (char*)malloc(sizeof(char) * (3));
		strcpy(cache, "Nil");
		depictor.funcdsp->formalscope->type = cache;
		cache = (char*)malloc(sizeof(char) * (3));
		strcpy(cache, "Nil");
		depictor.funcdsp->formalscope->name = cache;
	}
}

// 修改树的名称
void ChangeVariableName(GrammarTreeNode* g)
{
	char* cache = (char*)malloc(sizeof(char) * (17));
    strcpy(cache, "GlobalVariableDef");
    g->name = cache;
    
}

void ChangeFunctionName(GrammarTreeNode* g)
{
	char* cache = (char*)malloc(sizeof(char) * (17));
    strcpy(cache, "GlobalFunctionDef");
    g->name = cache;
}
...
void AddClassParent(GlobalScope global_symtable)
{
    ...
    {
        if (global_symtable->category == Class && global_symtable->depictor.classdsp->parentname)
        {
            tmp = first;
            while (tmp)
            {
				// 这个肯定要求类
                if (tmp->category == Class && !strcmp(global_symtable->depictor.classdsp->parentname, tmp->name))
                {
...
void DisplayLocalScope(LocalScope local_symtable, char* table)
{
    ...
    while(local_symtable)
    {
        ...
        printf("│%14s", local_symtable->type);
...
void DisplayGlobalVariable(LocalScope local_symtable, char* table)
{
    ...
    LocalScope first = local_symtable;
    if(!local_symtable) return;
    printf("%s Global Variable Symbol Table\n", table);
    printf("┌────────────┬────────┬────────────┬────────────┐\n");
    printf("│Name        │Category│Type        │EmbededScope│\n");
    if(local_symtable)
    {
...
void DisplayFormalScope(FormalScope formal_symtable, char* table)
{
    ...
    {
        printf("├────────────┼────────────────┼─────────┤\n");
        if(first->functionscope==NULL)
        	printf("│%12s│%16s│     Nil  |\n", formal_symtable->name, formal_symtable->type);
        else printf("│%12s│%16s│     ────┼────> %s Function Body Local Scope Symbol Table\n", formal_symtable->name, formal_symtable->type, table);
        formal_symtable = formal_symtable->next;
    }
    printf("└────────────┴────────────────┴─────────┘\n\n");
...
void DisplayGlobalScope(GlobalScope global_symtable, char* table)
{
    ...
    {
        printf("├────────────┼────────┼────────┤\n");
        printf("│%12s│", global_symtable->name);
		switch(global_symtable->category){
		case Class:
			printf("   Class│    ────┼───> %s Class Depictor\n",global_symtable->name);
			break;
		case Variable:
			printf("Variable│    Nil |\n");
			break;
		case Function:
			printf("Function|    ────┼───> %s Function Depictor\n",global_symtable->name);
			break;
		}
        global_symtable = global_symtable->next;
    }
    ...
    while (global_symtable)
    {
		switch(global_symtable->category){
			case Class:
				...
				break;
			case Function:
				DisplayFunctionDepictor(global_symtable->depictor.funcdsp, global_symtable->name);
				break;
			case Variable:
				DisplayGlobalVariable(global_symtable, global_symtable->name);
				break;
		}
        global_symtable = global_symtable->next;
    }
