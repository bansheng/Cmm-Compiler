...
%token <grammar_tree> KEYPUBLIC KEYPRIVATE KEYPROTECTED
...
%token <grammar_tree> KEYRETURN KEYCONTINUE
...
%type <grammar_tree> Program ProgramFileds ProgramFiled VariableDef Variable Type Variables Formals FunctionDef ClassDef Fields Field StmtBlock Stmts Stmt SimpleStmt LValue SelfLvalue Call Actuals Exprs ForStmt WhileStmt IfStmt ReturnStmt BreakStmt ContinueStmt PrintStmt BoolExpr Expr Constant
...
ProgramFileds: ProgramFiled { 
			 $$ = CreateGrammarTree("ProgramFileds", 1, $1); 
			 $$->globalscope = $1->globalscope;
			 }
			 | ProgramFileds ProgramFiled { 
			 $$ = CreateGrammarTree("ProgramFileds", 2, $1, $2);
			 $$->globalscope = MergeGlobalScope($1->globalscope, $2->globalscope);
			 }
			 ;

ProgramFiled: VariableDef { ChangeVariableName($1);
			$$ = CreateGrammarTree("ProgramFiled", 1, $1); 
			$$->globalscope = 
			AddIntoGlobal($$->globalscope, $1->localscope->name, Variable, $1->localscope->type, 0);/* 变量没有描述器 */ }
			| FunctionDef { ChangeFunctionName($1); 
			$$ = CreateGrammarTree("ProgramFiled", 1, $1); 
			
			Changethis($1->depictor);
			$$->globalscope = 
			AddIntoGlobal($$->globalscope, $1->localscope->name, Function, $1->classscope->type, 1, $1->depictor.classdsp);}
			| ClassDef { $$ = CreateGrammarTree("ProgramFiled", 1, $1); $$->globalscope = 
			AddIntoGlobal($$->globalscope, $1->lchild->rchild->string_value, Class, "class", 1, $1->depictor.classdsp);
			}
			;
...
FunctionDef: Type IDENTIFIER OPLEFTPRNT Formals OPRIGHTPRNT StmtBlock { 
           ...
           $5->formalscope = AddIntoFormal($5->formalscope, "Nil", "Nil", NULL);
...
Call: IDENTIFIER OPLEFTPRNT Actuals OPRIGHTPRNT { $$ = CreateGrammarTree("Call", 4, $1, $2, $3, $4); printf("普通函数调用0\n");}  
