...
/*        {*/
/*        case ScopeLocal:*/
/*            printf("local scope %s", scope.localscope->name);*/
/*            break;*/
/*        case ScopeFormal:*/
/*            printf("formal scope %s", scope.localscope->name);*/
/*            break;*/
/*        case ScopeClass:*/
/*            printf("class scope %s", scope.localscope->name);*/
/*            break;*/
/*        case ScopeGlobal:*/
/*            printf("global scope %s", scope.localscope->name);*/
/*            break;*/
/*        case ScopeGlobalVariable:*/
/*        	printf("global Variable %s", scope.localscope->name);*/
/*            break;*/
/*        case ScopeGlobalFunction:*/
/*        	printf("global Function %s", scope.localscope->name);*/
/*            break; */
/*    }*/
/*    printf(" into stack\n");*/
    ...
    while (--tmp >= stack.base)
    {
        switch (tmp->type)
        {
            ...
            case ScopeGlobal:
                ...
                break;
           	case ScopeGlobalVariable: // 全局变量
           		local_symtable = tmp->localscope;
           		if (local_symtable)
                {
                    if (!strcmp(local_symtable->name, name))
                    {
                        result.type = ScopeGlobalVariable;
                        result.localscope = local_symtable;
                        return result;
                    }
                }
            ...
            case ScopeGlobalFunction:
           		...
           		if (global_symtalbe)
                {
                    if (!strcmp(global_symtalbe->name, name))
                    {
                        result.type = ScopeGlobalFunction;
                        result.globalscope = global_symtalbe;
                        return result;
                    }
                }
...
ClassScope FindInClass(ScopeStack* stack, char* class_name, char* member, DecafCategory category)
{
    ...
    while (globalscope) //在主作用域里面找class的声明
    {
    ...
    if (globalscope && globalscope->depictor.classdsp->parent) //在父类中找这个成员，默认就是私有继承
    {
...
ExprType* GetExprType(GrammarTree tree, ScopeStack* stack)
{
    ...
    else if (!strcmp(tree->name, "IDENTIFIER"))
    {
        ...
        {
            result->category = scope.globalscope->category;
            result->type = scope.globalscope->type;
        }
        return result;
    }
    else
...
void ScopeTrial(GrammarTree tree, Scope scope, ScopeStack* stack)
{
    ...
    else if (!strcmp(tree->name, "ClassDef")) // 类
    {
        Scope result;
...
/*    else if (!strcmp(tree->name, "ProgramFiled") && !strcmp(tree->lchild->name, "FunctionDef"))*/
	else if (!strcmp(tree->name, "GlobalFunctionDef"))
    {
    	scope.type = ScopeGlobalFunction;
        scope.globalscope = tree->globalscope;
        PushScopeStack(stack, scope);
        ...
        if (!funcdsp->formalscope->functionscope) 
        {
...
/*    else if (!strcmp(tree->name, "ProgramFiled") && !strcmp(tree->lchild->name, "VariableDef"))*/
	else if (!strcmp(tree->name, "GlobalVariableDef"))
    {
    	scope.type = ScopeGlobalVariable;
    ...
    else if (!strcmp(tree->name, "LValue") && !strcmp(tree->lchild->name, "IDENTIFIER"))
    {
        ...
        else if (result.type == ScopeGlobal) // 类名不能作为变量名
        {
            smerror += 1;
    ...
    else if (!strcmp(tree->name, "LValue") && !strcmp(tree->lchild->name, "Expr") && !strcmp(tree->lchild->rchild->name, "SPDOT")) // 类的变量调用
    {
        ExprType* type = GetExprType(tree->lchild, stack);
        if (type)
        {
    ...
    else if (!strcmp(tree->name, "Call") && !strcmp(tree->lchild->name, "IDENTIFIER"))
    {
        ...
        else if (result.type == ScopeGlobalFunction)
        {
        	printf("普通函数调用成功\n");
        }
        ...
        else if (result.localscope->category != Function)
        {
            ...
            printf("Hint: You have Declared \"%s\" \n", tree->lchild->string_value);
    ...
    else if (!strcmp(tree->name, "Call") && !strcmp(tree->lchild->name, "Expr") && !strcmp(tree->lchild->rchild->name, "SPDOT")) // 类函数调用
    {
        ExprType* type = GetExprType(tree->lchild, stack);
        if (type)
        {
