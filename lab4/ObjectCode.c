...
int GetReg(FILE* fp, Scope variable)
{
    ...
    else if (variable.type == ScopeLocal || variable.type == ScopeGlobalVariable)
    {
        ...
        else
        {
            ...
            for (i = 8; i <= 25; i++)
            {
                if (RegDepictor[i].used == 0)
                {
                    ...
                    fprintf(fp, "sw %s, %d($fp)\n", RegName[i], RegDepictor[i].variable.localscope->offset); // 变量值先写回堆栈中
...
void GenerateObjectCode(FILE* fp, TACCode* entrance)
{
    ...
    if (marg == 0)
    {
        fprintf(fp, "ori $sp, $0, 1024\n");
        fprintf(fp, "ori $fp, $0, 1024\n");
    }
    fprintf(fp, "jal main\n");
    ...
    while (tmp)
    {
        ...
        if (tmp->code.opkind == ASSIGN && tmp->code.dest.kind == VARIABLE)
        {
            if (tmp->code.firstsrc.kind == VARIABLE)
                ...
                ;
            fprintf(fp, "sw %s, %d($fp)\n", RegName[GetReg(fp, tmp->code.dest.variable)], tmp->code.dest.variable.localscope->offset); // 写回栈桢
        }
        ...
        else if (tmp->code.opkind == FUNCTIONDF || tmp->code.opkind == GLOBALFUNCTIONDF)
        {
            int i = 0;
            int offset;
            TACCode* param = tmp;
            ...
            while (param->next->code.opkind == PARAM)
            {
                ...
                if (i < 3)
                {
                    param->code.dest.variable.formalscope->reg = i + 5;
                    RegDepictor[i + 5].variable = param->code.dest.variable;
                }
                else
        ...
        else if (tmp->code.opkind == CALLASSIGN)
        {
            ...
            while (arg->prev->code.opkind == ARG)
            {
                arg = arg->prev;
                if (i < 3)
                    fprintf(fp, "addu %s, %s, $0\n", RegName[i + 5], RegName[GetReg(fp, arg->code.dest.variable)]);
                else
                    fprintf(fp, "sw %s, -%d($sp)\n", RegName[GetReg(fp, arg->code.dest.variable)], (i - 3) * 4);
                i += 1;
            }
            if (i > 3)
            {
        ...
        else if (tmp->code.opkind == PRINTINT)
        {
        	fprintf(fp, "addu $a0, %s, $0\n", RegName[GetReg(fp, tmp->code.dest.variable)]);
            fprintf(fp, "ori $v0, $0, 34\n");
            fprintf(fp, "syscall\n");
