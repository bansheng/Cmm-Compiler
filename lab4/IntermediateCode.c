...
TACCode* TranslateStmt(GrammarTree tree, Scope scope, ScopeStack* stack)
{
    ...
    else if (!strcmp(tree->lchild->name, "ReturnStmt"))
    {
        ...
        else
        {
        	printf("空return语句\n");
        	TACCode* code2 = (TACCode*)malloc(sizeof(TACCode));
        	code2->code.opkind = RETURNVOID;
            code2->line = tree->lchild->line;
            code2->prev = code2;
            code2->code.dest.kind = INTEGERCONST;
            code2->code.dest.intvalue = 0;
            return code2;
        }
    }
    else if (!strcmp(tree->lchild->name, "PrintStmt"))
    {
        ...
        else if (!strcmp(expr->lchild->name, "LValue") && !strcmp(expr->lchild->lchild->name, "IDENTIFIER"))
        {
        	printf("不错,我要输出int了\n");
        	Scope new_temp;
            char cache[8];
...
/*    else if (!strcmp(tree->name, "ProgramFileds"))*/
/*    {*/
/*        if (!strcmp(tree->lchild->name, "ProgramFiled")) // 只剩最后一个全局域*/
/*        {*/
/*        	return GenerateInterCode(tree->lchild->lchild, scope, stack);*/
/*        }*/
/*        else // 两个全局域*/
/*        {*/
/*        	return MergeTACCode(2, GenerateInterCode(tree->lchild->lchild, scope, stack), */
/*                                GenerateInterCode(tree->lchild->lchild->rchild, scope, stack));*/
/*        }*/
/*            */
/*    }*/
    ...
    else if (!strcmp(tree->name, "GlobalVariableDef"))
    {
        ...
        // 插入全局变量
        TACCode* tmp = (TACCode*)malloc(sizeof(TACCode));
        tmp->code.opkind = GLOBALVARIABLE;
        tmp->code.dest.kind = VARIABLE;
        tmp->code.dest.variable = TraverseScopeStack(*stack, tree->lchild->lchild->rchild->string_value);
        tmp->line = tree->line;
        tmp->prev = tmp;
        return tmp;
    }
    ...
    else if (!strcmp(tree->name, "FunctionDef") || (!strcmp(tree->name, "GlobalFunctionDef")))
    {
    	if(!strcmp(tree->name, "GlobalFunctionDef"))
		{
...
/*        	printf("%s\n", tree->lchild->rchild->rchild->string_value);*/
    ...
    else if (!strcmp(tree->name, "StmtBlock"))
    {
        ...
        //According the translation rule of c, the second parameter "GenerateInterCode" will operate first
        //The first parameter "GenerateInterCode" is used to traverse "SPRIGHTBRACE"
        return MergeTACCode(2, GenerateInterCode(tree->lchild->rchild->rchild, scope, stack), GenerateInterCode(tree->lchild->rchild, scope, stack));
    }
    ...
    while (tmp)
    {
        ...
        else if (tmp->code.opkind == ADD | tmp->code.opkind == SUB | tmp->code.opkind == MUL | tmp->code.opkind == DIV)
        {
            ...
            if (tmp->code.firstsrc.kind == VARIABLE)
                ...
                printf("#%f ", tmp->code.firstsrc.floatvalue);
        ...
        else if (tmp->code.opkind == GLOBALFUNCTIONDF)
            printf("\n%3d  (at line %3d)\tGLOBALFUNCTIONDF %s :\n", line, tmp->line, tmp->code.dest.function.classscope->name);
        else if (tmp->code.opkind == GLOBALVARIABLE)
/*            printf("\n%3d  (at line %3d)\tGLOBALVARIABLE %s\n", line, tmp->line, tmp->code.dest.variable.formalscope->name);*/
		{
			// do nothing
		}
        ...
        else if (tmp->code.opkind == RETURNVOID)
        {
            printf("%3d  (at line %3d)\tRETURNVOID \n", line, tmp->line);
        }
